...
Assuming $F_t$ is invertible for all $t$ (this restriction is relaxed in Appendix F of the supplementary materials), then $\theta_t = F_t^{-1}(y_t - L_V\psi_t)$ for $t=1,2,\cdots,T$ while $\theta_0=\psi_0$. Define $\mu_1 = L_V\psi_1 + F_1G_1\psi_0$ and $\mu_t =L_V\psi_t + F_tG_tF_{t-1}^{-1}(y_{t-1} - L_{V}\psi_{t-1})$ for $t=2,3,\cdots,T$. Then the scaled error parameterization is 
\begin{align*}
...
\item[] Appendix L: Additional plots for other values of $T$.
\item[] Appendix M: Comparing GIS and Alt in very long time series.
\end{itemize}
