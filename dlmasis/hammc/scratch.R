nuts_da <- function(f, M, Madapt, theta0, delta=0.6){
  ...
  for(m in  2:(M+Madapt)){
    ...
    while(s == 1){
      ...
      if(v == -1){
        BTout <- build_tree(thetaminus, rminus, gradminus, logu, v, j, epsilon, f, joint)
        thetaminus <- BTout[[1]]
        rminus <- BTout[[2]]
        gradminus <- BTout[[3]]
        thetaprime <- BTout[[7]]
        ...
        BTout <- build_tree(thetaplus, rplus, gradplus, logu, v, j, epsilon, f, joint);
        thetaplus <- BTout[[4]]
        rplus <- BTout[[5]]
        gradplus <- BTout[[6]]
        thetaprime <- BTout[[7]]
...
build_tree <- function(theta, r, grad, logu, v, j, epsilon, f, joint0){
  if (j == 0){
    ...
    lfout <- leapfrog(theta, r, grad, v*epsilon, f)
    ...
    BTout <- build_tree(theta, r, grad, logu, v, j-1, epsilon, f, joint0)
    thetaminus <- BTout[[1]]
    rminus <- BTout[[2]]
    gradminus <- BTout[[3]]
    thetaplus <- BTout[[4]]
    ...
    if(sprime == 1){
      if(v == -1){
        BTout <- build_tree(thetaminus, rminus, gradminus, logu, v, j-1, epsilon, f, joint0)
        thetaminus <- BTout[[1]]
        rminus <- BTout[[2]]
        gradminus <- BTout[[3]]
        thetaprime2 <- BTout[[7]]
        ...
        BTout <- build_tree(thetaplus, rplus, gradplus, logu, v, j-1, epsilon, f, joint0);
        thetaplus <- BTout[[4]]
        rplus <- BTout[[5]]
        gradplus <- BTout[[6]]
        thetaprime2 <- BTout[[7]]
...
find_reasonable_epsilon <- function(theta0, grad0, logp0, f){
  ...
  lfout <- leapfrog(theta0, r0, grad0, epsilon, f)
  rprime <- lfout[[2]]
  logpprime <- lfout[[4]]
