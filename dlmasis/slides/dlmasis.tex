...
\begin{itemize}
  \item  GIS has a geometric rate of convergence no worse than the worst of the two underlying DA algorithms, and the bound gets better the less $\theta$ and $\gamma$ are correlated in the posterior.\\~\\
...
\end{align*}\\~\\
But the target posterior is
\begin{align*}
p(\phi|y)&\propto(\phi)|\tilde{V} + \tilde{W} + \tilde{C}|^{-1/2}\\
&\times\exp\left[-\frac{1}{2}\left(y-D\tilde{m}\right)'(\tilde{V} + \tilde{W} + \tilde{C})^{-1}\left(y-D\tilde{m}\right)\right]
\end{align*}
\end{frame}
