...
\end{align}
where again $v_t=y_t - F_t\theta_t$ and $w_t=\theta_t - G_t\theta_{t-1}$ for $t=1,2,\cdots,T$, $tr(.)$ is the matrix trace operator, $p=dim(\theta_t)$, and $k=dim(y_t)$.  The transformation from $\theta_{0:T}$ to $w_{0:T}$ implies that $\theta_t = \sum_{s=0}^{t} H^t_{s+1}w_{s}$  for $t=1,2,\cdots,T$, where we define $H^t_s=G_tG_{t-1}\cdots G_s$ for $s=1,2,\cdots,t$ and $t=1,2,\cdots,T$, and $H_{t+1}^t=I_p$, the $p\times p$ identity matrix. For $t=0$ we also have $\theta_0=\gamma_0$. The Jacobian of the transformation is then upper triangular with ones along the diagonal, so it's determinant is one. Now we can write the joint posterior of $(V_{1:T},W_{1:T},w_{0:T})$ as
\begin{align*}
  ...
  & \times |V_t|^{-(\eta_{t} + k + 3)/2} \exp\left[-\frac{1}{2}tr\left(\left[\Psi_{t} + \left(y_t - F_t\textstyle\sum_{s=0}^{t}H^t_{s+1}w_{s}\right)\left(y_t - F_t\textstyle\sum_{s=0}^{t}H^t_{s+1}w_{s}\right)'\right]V_t^{-1}\right)\right].\nonumber\\
\end{align*}
...
\begin{align*}
  y_t|w_{0:T}& \stackrel{ind}{\sim} N\left(F_t\textstyle\sum_{s=0}^{t}H_{s+1}^tw_{s},V_t\right)\\
...
Define the scaled disturbances in the general DLM, $\gamma_{0:T}$, where $\gamma_0=\theta_0=w_0$ and for $t=1,2,\cdots,T$, $\gamma_t = L_t(\theta_t - G_t\theta_{t-1}) = L_tw_t$ where $L_t$ is the Cholesky decomposition of $W_t^{-1}$, i.e. $L_t' L_t =W_t^{-1}$. We can confirm our intuition that the scaled disturbances are an AA for $V_{1:T}$ and $W_{1:T}$ jointly. Let $L_0=I_p$, the $p\times p$ identity matrix. Then $\theta_t = \sum_{s=0}^{t}H^t_{s+1}L_{s}^{-1}\gamma_{s}$ for $t=1,2,\cdots,T$ and $\theta_0=\gamma_0$. Then the Jacobian is block upper triangular with $L_t^{-1}$'s as the diagonal blocks, so $|J|=\prod_{t=1}^T|L_t^{-1}|=\prod_{t=1}^T|W_t|^{1/2}$. Then from \eqref{dlmjoint} we can write the full joint distribution of $(V_{1:T},W_{1:T},\gamma_{0:T},y_{1:T})$ as
 \begin{align}
  ...
  &\times  |V_t|^{-1/2}\exp\left[-\frac{1}{2}\left(y_t - F_t\textstyle\sum_{s=0}^{t}H^t_{s+1}L_{s}^{-1}\gamma_{s}\right)'V_t^{-1}\left(y_t - F_t\textstyle\sum_{s=0}^{t}H^t_{s+1}L_{s}^{-1}\gamma_{s}\right)\right]\nonumber\\
...
\begin{align*}
  y_t|\gamma_{0:T},V_{1:T},W_{1:T} & \stackrel{ind}{\sim} N\left(F_t\textstyle\sum_{s=0}^{t}H^t_{s+1}L_{s}^{-1}\gamma_{s}, V_t\right)\\
...
\end{align*}
where $\nu_t = y_t - F_t\textstyle\sum_{s=0}^{t}H^t_{s+1}(L^{(k+0.5)}_{s})^{-1}\gamma_{s}$. In other words, independent inverse Wishart distributions with parameters $\Psi_t + \nu_t\nu_t'$ and $\eta_t + 1$. Now $\theta_t = \textstyle\sum_{s=0}^{t}H^t_{s+1}(L^{(k+0.5)}_{s})^{-1}\gamma_{s}$ because this is just the back transform of step 4 of Algorithm \ref{pcisalg} or step 3 of Algorithm \ref{sdintalg}, so $\nu_t=v_t$ and thus the distributions are the same. As a result, we can either only draw $W_{1:T}$ in step 2 or step 4 can be ommitted. 
...
First consider $\tilde{\gamma}_{0:T}$. If we define $L_0=K_0=I_k$ where $I_k$ is the $k\times k$ identity matrix, then $\tilde{\gamma}_t = K_tL_t^{-1}\gamma_t$ for $t=0,1,2,\cdots,T$. The reverse transformation is then $\gamma_t = L_tK_t^{-1}\tilde{\gamma}_t$. The Jacobian is then block diagonal with $L_tK_t^{-1}$'s along the diagonal. Thus $|J|=\prod_{t=0}^T|L_t||K_t|^{-1}=\prod_{t=1}^T|W_t|^{-1/2}|V_t|^{1/2}$. Then from \eqref{dlmdistjoint} we can write the joint distribution of $(V_{1:T},W_{1:T},\tilde{\gamma}_{0:T},y_{1:T})$ as
 \begin{align}
  ...
  &\times  |W_t|^{-1/2}\exp\left[-\frac{1}{2}\left(y_t - F_t\textstyle\sum_{s=0}^{t}H^t_{s+1}K_{s}^{-1}\tilde{\gamma}_{s}\right)'V_t^{-1}\left(y_t - F_t\textstyle\sum_{s=0}^{t}H^t_{s+1}K_{s}^{-1}\tilde{\gamma}_{s}\right)\right]\nonumber\\
 ...
 \end{align}
where again $H^t_s=G_tG_{t-1}\cdots G_s$ for $s=1,2,\cdots,t$ and $H_{t+1}^t=I_k$. Then under $\tilde{\gamma}_{0:T}$ we can write the model as
\begin{align*}
  y_t|\tilde{\gamma}_{0:T},V_{1:T},W_{1:T} & \stackrel{ind}{\sim} N\left(F_t\textstyle\sum_{s=0}^{t}H^t_{s+1}K_{s}^{-1}\tilde{\gamma}_{s}, V_t\right)\\
