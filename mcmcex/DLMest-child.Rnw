...
To start with, we'll rewrite \eqref{dlmjoint} in a less messy form. Define the following matrices:
\begin{align*}
  V=\begin{bmatrix} I_{k} & 0 & \cdots & 0 \\
    0 & V_1 & \cdots & 0 \\
    \vdots & \vdots & \ddots & \vdots \\
    0 & 0 & \hdots & V_T\end{bmatrix}, && W=\begin{bmatrix} C_0 & 0 & \cdots & 0 \\
    0 & W_1 & \cdots & 0 \\
    \vdots & \vdots & \ddots & \vdots \\
    0 & 0 & \hdots & W_T\end{bmatrix}, \\
  G = \begin{bmatrix} 0 & 0 & \hdots & 0 & 0\\
    G_1 & 0 & \hdots &0& 0\\
    0 & G_2 & \hdots &0& 0\\
    \vdots & \vdots & \ddots &\vdots& \vdots \\
    0 & 0 & \hdots & G_T & 0 \end{bmatrix}, &&   F = \begin{bmatrix} 0 & 0 & 0 & \hdots & 0 \\
    0 & F_1 & 0 &  \hdots &0\\
    0 & 0 & F_2 & \hdots &0\\
    \vdots & \vdots & \vdots &\ddots& \vdots \\
    0 & 0 & 0 & \hdots & F_T \end{bmatrix},
\end{align*}
\begin{align*}
  \theta = \begin{bmatrix} \theta_0 \\ \theta_1 \\ \vdots \\ \theta_T \end{bmatrix}, && y = \begin{bmatrix} 0 \\ y_1 \\ y_2 \\ \vdots \\ y_T \end{bmatrix},&&\mathrm{and}\  m = \begin{bmatrix} m_0 \\ 0 \\ \vdots \\ 0 \end{bmatrix}
\end{align*}
where $I_k$ is the $k\times k$ identity matrix. With the notation so defined, we can write the full joint distribution of data and parameters as
\begin{align}
  p(V_{1:T}&,W_{1:T},\theta_{0:T},y_{1:T}) \propto p(V_{1:T})p(W_{1:T}) \nonumber \\
  &\times |V|^{-1/2}\exp\left[-\frac{1}{2}(y-F\theta)'V^{-1}(y-F\theta)\right]|W|^{-1/2}\exp\left[-\frac{1}{2}(\theta-G\theta)'W^{-1}(\theta-G\theta)\right]\label{dlmmatjoint}
\end{align}

Now we can define the scaled disturbances in the general DLM. Let $L_t$ denote the Cholesky decomposition of $W_t^{-1}$, i.e. $L_t' L_t =W_t^{-1}$, and define 
\begin{align*}
  L = & \begin{bmatrix} L_0 & 0 & \hdots & 0 \\
    0 & L_1 & \hdots & 0 \\
    \vdots & \vdots & \ddots & \vdots \\
    0 & 0 & \hdots & L_T \end{bmatrix}.
\end{align*}
So we have $L'L = W^{-1}$. Then we'll define the scaled disturbances as $\gamma = L[(I_{(T+1)p}-G)\theta - m]$ where again $I_{n}$ is the $n\times n$ identity matrix. In other words $\gamma_0=\theta_0-m_0$ and for $t=1,2,\cdots,T$, $\gamma_t = L_t(\theta_t - G_t\theta_{t-1})$.  We can confirm our intuition that the scaled disturbances are an AA for $V_{1:T}$ and $W_{1:T}$ jointly. The reverse transformation is $\theta = (I_{(T+1)p}-G)^{-1}(L^{-1}\gamma + m)$. Then the Jacobian is $J = (I_{(T+1)p} - G)^{-1}L^{-1}$ so $|J| = |L|^{-1}|I_{(T+1)p} - G|^{-1} =|W|^{-1/2}$ since $I_{(T+1)p} - G$ is block lower triangual with identity matrices in the diagonal blocks. Then from \eqref{dlmmatjoint} we can write the full joint distribution of $(V_{1:T},W_{1:T},\gamma_{0:T},y_{1:T})$ as
 \begin{align}
  p(&V_{1:T},W_{1:T},\gamma_{0:T},y_{1:T}) \propto p(V_{1:T})p(W_{1:T})\exp\left[-\frac{1}{2}\gamma'\gamma\right] \nonumber\\
  &\times \exp\left[-\frac{1}{2}\left[y-F(I_{(T+1)p}-G)^{-1}(L^{-1}\gamma + m)\right]'V^{-1}\left[y-F(I_{(T+1)p}-G)^{-1}(L^{-1}\gamma + m)\right]\right]. \label{dlmdistjoint}
 \end{align}
