...
To complete the model specification in a Bayesian context, we need priors on $\theta_0$, $V_{1:T}$, and $W_{1:T}$. For simplicity we'll assume that they're mutually independent a priori and that $\theta_0 \sim N(m_0, C_0)$, $V_t \sim IW(\Psi_{t}, \eta_{t})$ for $t=1,2,\cdots,T$, and $W_t \sim IW(\Omega_{t}, \delta_{t})$ for $t=1,2,\cdots,T$ where $m_0$, $C_0$ and $\Psi_{t}$, $\eta_{t}$, $\Omega_{t}$, and $\delta_{t}$ for $t=1,2,\cdots,T$ are known hyperparameters  and $IW(\Psi, \eta)$ denotes the inverse Wishart density with degrees of freedom $\eta$ and positive definite scale matrix $\Psi$. 
...
In any case, the inverse wishart priors on $V_{1:T}$ and $W_{1:T}$ and the normal prior on $\theta_0$ allow us to write the full joint distribution of $(V_{1:T},W_{1:T},\theta_{0:T},y_{1:T})$ as
\begin{align}
