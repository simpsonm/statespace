...
situations. However, as we'll see in the next section, both samplers
are rather slow so interweaving them might not be very efficient. It
may the case that each of the 3 available interweaving algorithms is
fastest to converge (or more efficient) for different parameter
values. In that case, there may be an algorithm the somehow hybridizes
the interweaving algorithms in order to come up with something
better. Another possibility worth noting is some sort of 3-way
interweaving algorithm.
