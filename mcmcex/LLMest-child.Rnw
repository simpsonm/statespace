...
<<baseESplot, fig.cap=cap, echo=FALSE, fig.height=4, fig.width=6, out.width=".48\\textwidth">>=
...
<<baseintESplot, fig.cap=cap, echo=FALSE, fig.width=10, fig.height=3.5, outwidth=".8\\textwidth">>=
...
Figure \ref{baseintESplot} contains plots of ESP for $V$ and $W$ in each of the base and GIS samplers and the full CIS sampler for $T=10$, $T=100$, and $T=1000$. We can see several of our predictions borne out. The state-dist GIS algorithm has a high ESP for $V$ when $W/V$ is high or near 1 and for $W$ when $W/V$ is near 1, but otherwise has a low ESP. Similarly, the state-error GIS algorithm has a high ESP for $W$ when $W/V$ is low or near 1 and for $V$ when $W/V$ is near 1, but otherwise has a low ESP. The dist-error GIS algorithm has a high ESP for $V$ and $W$ when $W/V$ is near 1, but otherwise has a low ESP. The full CIS algorithm also looks identical to this. Our only preidction that isn't borne out is the superiority of the triple GIS algorithm --- it also looks identical to the dist-error algorithm.

<<hybridESplot, fig.cap=cap, echo=FALSE, fig.height=4, fig.width=5.5, out.width=".49\\textwidth">>=
