...
tripleinter <- function(n, start, dat, Qv=1000, Qw=1000, inter=c(TRUE, TRUE)){
  T <- length(dat)
  V <- start[1]
  W <- start[2]
  ...
  for(i in 1:n){
    ...
    Vout <- Vgamiter(dat, gam, W, Qv)
    sigv <- Vout[2]
    V <- Vout[1]
    Wout <- Wgamiter(dat, gam, sigv, Qw)
    sigw <- Wout[2]
    W <- Wout[1]
    ## scaled error sampler
    if(!inter[2]){
...
VWthetaiter <- function(dat, theta, Qv, Qw){
  T <- length(dat)
  V <- rgig(1, sum((dat-theta[-1])^2), Qv, -(T-1)/2)
  W <- rgig(1, sum((theta[-1]-theta[-(T+1)])^2), Qw, -(T-1)/2)
  sigv <- sqrt(V)*(2*rbinom(1,1,1/2)-1)
  sigw <- sqrt(W)*(2*rbinom(1,1,1/2)-1)
  return(c(V,W,sigv,sigw))
}
...
Wgamiter <- function(dat, gam, V, Qw){
  ...
  cgam <- cumsum(gam[-1])
  gam0 <- gam[1]
  Sighatinv <- 1/Qw + 1/V*sum(cgam^2)
  Sighat <- 1/Sighatinv
  muhat <- Sighatinv/V*sum( (dat - gam0)*cgam )
...
Vpsiiter <- function(dat, psi, W, Qv){
  ...
  Sighat <- 1/Sighatinv
  muhat <- Sighatinv/W*sum(Dys*Dpsistar)
