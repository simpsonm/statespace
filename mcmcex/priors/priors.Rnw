...
But this isn't the only way we could have written the model. Instead of using the $\theta_t$'s as our latent states, we might choose to use the scaled disturbances. Define $\gamma_0=\theta_0$ and $\gamma_t = L_t(\theta_t - G_t\theta_{t-1}) = L_tw_t$ for $t=1,2,\cdots,T$ where $L_t$ is the Cholesky decomposition of $W_t^{-1}$, i.e. $L_t'L_t = W_t^{-1}$. Loosely speaking we are taking the system disturbances and scaling them by their standard deviation. This parameterization of the DLM is well known and data augmentation algorithm based on it has different properties than the data augmentation algorithm based on the $\theta_t$'s, see e.g. \cite{fruhwirth2004efficient}. In particular if we consider the local level model, i.e. univariate $y_t$ and $\theta_t$ with $F_t=G_t=1$, $V_t=V$ and $W_t=W$, when $W$ is small relative to $V$, then the standard DA algorithm based on $\theta_{0:T}$ will have poor mixing for 
\end{document}


Drawing an analogy between non-dynamic hierarchical models and dynamic linear models, \cite{gelman2006prior} would suggest that this prior is a poor choice if the goal is ``noninformativeness''. This may be true, though the DLM isn't the same as a typical hierarchical model. Following \citeauthor{gelman2006prior}'s advice, we might try a half-$t$ distribution each standard deviation parameter and a uniform distribution for each correlation parameter in the covariance matrices. It turns that that this prior can be written as a mixutre of inverse Wishart priors on the covariance matrix, with $k$ independent inverse gamma mixing distributions (\cite{huang2013simple}). The upshot for our purposes is that with the suggested noninformative prior of \citeauthor{huang2013simple}, the MCMC algorithms we discuss below only need to be changed to add two more steps to draw the inverse gamma mixing distributions for both covariance matrices $V$ and $W$. We focus on the inverse Wishart prior primarily for simplicity of exposition.
