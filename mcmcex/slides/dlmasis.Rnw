...
\begin{frame}[fragile]
  \frametitle{The Dynamic Linear Model} 
  The $\theta$'s form a markov chain and conditional on the $\theta$'s, the $y$'s are mutually independent.\\~\\
\begin{figure}
  \centering
...
\begin{align*}
  ...
  \end{align*} 
  with $v_t\stackrel{iid}{\sim}N(0,V)$, $w_t\stackrel{iid}{\sim}N(0,W)$ and $\theta_0\sim N(m_0,C_0)$, all mutually independent.\\~\\
  \begin{itemize}%
  \item $\theta_t=E[y_t|\theta_t]$; i.e. the ``level'' in period $t$, and evolves over time through a random walk.\\~\\
...
In general, let $y$ denote the data vector and $\phi$ the parameter vector. Goal: use MCMC to obtain a markov chain with stationary distribution $p(\phi|y)$.\\~\\

Data Augmentation (DA) algorithm: let $\theta$ denote an {\it \color{red} augmented data vector} such that $p(\theta,\phi)$ is well defined. Given iteration $\phi^{(k)}$:\\~\\
\begin{enumerate}
...
\begin{enumerate}
  ...
  \item (Ancillary-Sufficiency) Interweaving
\end{enumerate}
\end{frame}

\begin{frame}
  \frametitle{Alternative Data Augmentations}
In general, let $y$ denote the data, $\phi$ denote the model parameters, and $\theta$ denote a data augmentations. Then:
...
\citet{papaspiliopoulos2007general}: DA based on a SA and DA based on an AA will typically have poor mixing and convergence in opposite regions of the parameter space.
\begin{itemize}
...
\item Option 3: Ancillaryity-Sufficiency Interweaving Strategies (ASIS) of \citet{yu2011center}.
\end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \citet{yu2011center}: 
  \begin{enumerate}
  \item  GIS has a geometric rate of convergence no worse than the worst of the two undelying DA algorithms, and the bound gets better the less $\theta$ and $\tilde{\theta}$ are correlated in the posterior.
  ...
  \item If $\theta$ is a SA, $\tilde{\theta}$ is an AA, $\tilde{\theta}$ is a one-to-one transformation of $\theta$, and the priors are ``nice,'' then the GIS algorithm is has the optimal convergence rate (in a certain class of algorithms).
  \end{enumerate}
\end{frame}
...
\begin{frame}
  ...
  \begin{itemize}
   ...
   \item If $\theta$ and $\phi$ are highly correlated in the posterior, drawing $p(\theta|\phi,y)$ then $p(\phi|\theta,y)$ won't move the chain much.
   ...
   \item If the DA algorithms based on $\theta$ and $\tilde{\theta}$ have high yield chains with high autocorrelations in opposite regions of the parameter space, interweaving algorithm ensures that at least one step moves the chain significantly.\\~\\
  \end{itemize}
...
Sometimes it's not possible to find an SS-AA pair of augmentations for $\phi$, but if $\phi=(\phi_1, \phi_2)$, it's possible to do componentwise interweaving while still taking advantage of SA-AA pairs.\\~\\
...
\end{align*}
so clearly $\theta_{0:T}$ is nether SA nor AA for $(V,W)$, but it is SA for $W|V$ and AA for $V|W$.\\~\\

Recall the sampler based on $\theta_{0:T}$ is called the {\it \color{red} state sampler}.\\~\\

Other obvious DAs: 
\begin{enumerate}
  \item The disturbances $(w_{0:T})$ where $w_0\equiv \theta_0$. Results in the state sampler --- $p(V,W|\theta_{0:T},y_{1:T})=p(V,W|w_{0:T},y_{1:T})$.
...
Less obvious: try scaling the errors/disturbances by their standard deviation. Define the scaled disturbances: $\gamma_0=\theta_0$ and for $t=1,2,...,T$ 
    \[
    \gamma_t =\frac{\theta_t - \theta_{t-1}}{\sqrt{W}}
    \]
...
$\psi_{0:T}$ appears to be a new parameterization.
\end{frame}
...
\begin{frame}
  \frametitle{Two New DA Algorithms}
...
\item $V\sim IG(\alpha_V, \beta_V)$ with $\alpha_V=5$ and $\beta_V=(alpha_V - 1)\tilde{V}$ where $\tilde{V}$ is the value of $V$ used to simulated the dataset.
\item $W\sim IG(\alpha_W, \beta_W)$ with $\alpha_W=5$ and $\beta_W=(alpha_W - 1)\tilde{W}$ where $\tilde{W}$ is the value of $W$ used to simulated the dataset.\\~\\
\end{itemize}
...
To test these parameterizations, I simulated a fake data set from the local level model for each $(V,W)$ pair over a grid and for each of $T=10,100,1000$. Then the model was fit using all three samplers with starting values $(\tilde{V},\tilde{W})$.\\~\\

\end{frame}

\begin{frame}
  \frametitle{Evaluating the Algorithms: Effective Sample Size and Effective Sample Proportion}
  Suppose we want to estimate $\mathrm{E}[V|y_{1:T}]$ with 
  \[
  \hat{V}\equiv\frac{\textstyle\sum_{k=1}^KV^{(k)}}{K}
  \]
  If we had $K$ iid draws, the CLT would give 
  \[
  \mathrm{Var}(\hat{V})=\frac{\sigma_V^2}{K}
  \]
where $\sigma_V^2=\mathrm{Var}(V|y_{1:T})$. For our markov chain 
\[
\mathrm{Var}(\hat{V})=\frac{\sigma^2_V}{ESS}
\]
where $ESS$ is the ``effective sample size.'' $ESS$ is estimated by fitting an $AR(p)$ model to the chain. \\~\\
  
  The effective sample proportion is the effective sample size as a proportion of the actual sample size, i.e. $ESS=ESP/K$.
\end{frame}
...
\begin{frame}
  ...
  The state sampler agrees with the scaled disturbance sampler about $V$ and withthe scaled error sampler about $W$. It's at it's best for $(V,W)$ when the signal-to-noise ratio is near $1$.\\~\\
  
  Despite not being an SA-AA pair, $\gamma_{0:T}$ and $\psi_{0:T}$ make a nice beauty and the beast pair.\\~\\
  
  As we'll see, $ESP$s decrease as $T$ increases.
\end{frame}
...
\begin{frame}
  ...
  \end{enumerate}
  Note that we have to use $\gamma_{0:T}$ in step 4, otherwise steps 5 and 3 would be identical because $p(W|V,\theta_{0:T},y_{1:T})=p(W|V,\psi_{0:T},y_{1:T})$.
\end{frame}
...
\begin{frame}
  ...
  For $t=1,2,...,T$ define
  \begin{align*}
    \tilde{\gamma}_t & = \frac{\sqrt{W}}{\sqrt{V}}\gamma_t\\
  \end{align*}
...
\end{align*}
$\tilde{\gamma}_{0:T}$ is a SA for $W|W$. So $\gamma_{0:T}$ \& $\tilde{\gamma}_{0:T}$ make a AA-SA pair for $W|V$
\end{frame}
...
\begin{frame}
  ...
  For $t=1,2,...,T$ define
  \begin{align*}
    \tilde{\psi}_t & = \frac{\sqrt{V}}{\sqrt{W}}\psi_t\\
  \end{align*}
...
$\tilde{\psi}_{0:T}$ is a SA for $V|W$. So $\psi_{0:T}$ \& $\tilde{\psi}_{0:T}$ make a AA-SA pair for $V|W$
\end{frame}
...
\begin{enumerate}
  ...
  \item Draw $W$ from $p(W|V^{(k+1)}, theta_{0:T}, y_{1:T})$.
...
Possibly use metropolis steps for $(V,W)$ jointly in the Dist-Error GIS sampler --- untested, but likely has decent mixing properties.
\end{frame}
