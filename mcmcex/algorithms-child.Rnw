...
scaled so that they have a $N(0,1)$ distribution a priori. A similar parameterization is used in a dynamic regression state space model, where the regression parameter is an AR(1) process, in \citet{fruhwirth2004efficient}.
