...
@
\caption{Trace plots for $log(V)$ in the state sampler.}
\label{plotStateV}
\end{figure}

\begin{figure}[htb]
  \centering
<<SWtrplots, echo=FALSE>>=
par(mfrow=c(3,3))
for(j in 1:3){
  for(i in 1:3){
    nam <- paste(paste("V=", Vs[i], sep=""), paste("W=", Ws[j], sep=""), sep=", ")
...
rm(ssam)
@
\caption{Trace plots for $log(W)$ in the state sampler.}
\label{plotStateW}
\end{figure}
...
@
\caption{Trace plots for $log(V)$ in the scaled disturbance sampler.}
\label{plotDistV}
\end{figure}

\begin{figure}[htb]
  \centering
<<DWtrplots, echo=FALSE>>=
par(mfrow=c(3,3))
for(j in 1:3){
  for(i in 1:3){
    nam <- paste(paste("V=", Vs[i], sep=""), paste("W=", Ws[j], sep=""), sep=", ")
...
rm(dsam)
@
\caption{Trace plots for $log(W)$ in the scaled disturbance sampler.}
\label{plotDistW}
\end{figure}

\begin{figure}[tb]
  \centering
<<EVtrplots, echo=FALSE>>=
load("errorsam.RData")
...
@
\caption{Trace plots for $log(V)$ in the scaled error sampler.}
\label{plotErrorV}
\end{figure}

\begin{figure}[htb]
  \centering
<<EWtrplots, echo=FALSE>>=
par(mfrow=c(3,3))
for(j in 1:3){
  for(i in 1:3){
    nam <- paste(paste("V=", Vs[i], sep=""), paste("W=", Ws[j], sep=""), sep=", ")
...
rm(esam)
@
\caption{Trace plots for $log(W)$ in the scaled error sampler.}
\label{plotErrorW}
\end{figure}
