...
\matt{Do we need to use knitr? I suggest we turn this into a straight LaTeX document. Likely the publishers will want a LaTeX document anyway (although I realize you could use the intermediate from a knitr compilation). If you use include for the subdocuments then includeonly is a convenient command to only compile one section at a time. But, I think we might also want to just make this a single document. This will make some changes easier, e.g. global replacements such as ``a SA'' to ``an SA'' or finding whether a particular equation is referenced anywhere.}
<<intro-child, child='Intro/intro.Rnw', eval=TRUE>>=
@ 

<<DLMest-child, child='DLMest/DLMest.Rnw', eval=TRUE>>=
@ 

<<LLMest-child, child='LLMest/LLMest.Rnw', eval=TRUE>>=
@ 

<<<discussion-child, child='Discuss/discuss.Rnw', eval=TRUE>>=
@ 

\appendix
<<appendix-child, child='Appendix/appendix.Rnw', eval=TRUE>>=
@ 
