...
\begin{align*}
  p(W_{1:T}&|\gamma_{0:T},\cdots)  \propto \exp\left[-\frac{1}{2}\sum_{t=1}^T\left(y_t - F_tH_{0t}\gamma_0 - F_t\sum_{s=1}^tH_{st}L_s\gamma_s\right)'(K_t^{-1})'K_t^{-1}\left(y_t - F_tH_{0t}\gamma_0 - F_t\sum_{s=0}^tH_{st}L_s\gamma_s\right)\right]\\
  &\propto \exp\left[-\frac{1}{2}\sum_{t=1}^T\left(\left(\sum_{s=1}^t\gamma_s'L_s'H_{st}'\right)F_t'(K_t^{-1})'K_t^{-1}F_t\sum_{s=1}^tH_{st}L_s\gamma_s - 2(y_t - F_tH_{0t}\gamma_0)'(K_t^{-1})'K_t^{-1}F_t\sum_{s=1}^tH_{st}L_s\gamma_s\right)\right]\\
  &\propto \exp\left[-\frac{1}{2}\sum_{t=1}^T\left(\left(\sum_{s=1}^t\gamma_s'L_s'H_{st}'\right)A_t'A_t\sum_{s=1}^tH_{st}L_s\gamma_s - 2b_t'A_t\sum_{s=1}^tH_{st}L_s\gamma_s\right)\right]
\end{align*}
where we define $b_t = K_t^{-1}(y_t - F_tH_{0t}\gamma_0)$ and $A_t = K_t^{-1}F_t$. 

This doesn't look like it has any conjugate form for $W_t$, but with some finesse it looks like it might be a normal kernel for $L_t$, the Cholesky decomposition of $W_t$. Then since we can assume the Jacobian from $W_t\to L_t$ will be absorbed into the prior, which we are ignoring, we have
\begin{align*}
    p(L_{1:T}|\gamma_{0:T},\cdots) &\propto \exp\left[-\frac{1}{2}\sum_{t=1}^T\left(\sum_{s=1}^t\gamma_s'L_s'H_{st}'A_t'A_tH_{st}L_s\gamma_s + 2\sum_{r<s}^t \gamma_r'L_r'H_{rt}'A_t'A_tH_{st}L_s\gamma_s- 2b_t'A_t\sum_{s=1}^tH_{st}L_s\gamma_s\right)\right]\\
    &\propto \exp\left[-\frac{1}{2}\sum_{t=1}^T\sum_{s=1}^t\left(\gamma_s'L_s'B_{st}'B_{st}L_s\gamma_s - 2\left(b_t' - \sum_{r=1}^{s-1} \gamma_r'L_r'B_{rt}'\right)B_{st}L_s\gamma_s\right)\right]\\
    &\propto \exp\left[-\frac{1}{2}\sum_{t=1}^T\sum_{s=t}^T\left(\gamma_t'L_t'B_{ts}'B_{ts}L_t\gamma_t - 2\left(b_s' - \sum_{r=1}^{t-1} \gamma_r'L_r'B_{rs}'\right)B_{ts}L_t\gamma_t\right)\right]\\
\end{align*}
where we define $B_{st}=A_tH_{st}$. Now let's define
\begin{align*}
  C_t =& \sum_{s=t}^TB_{ts}'B_{ts} = \sum_{s=t}^TH_{st}'A_t'A_tH_{st} = \sum_{s=t}^T \left(K_t^{-1}F_t\prod_{r=s+1}^tG_r\right)'\left(K_t^{-1}F_t\prod_{r=s+1}^tG_r\right)\\
  \intertext{and}
    c_t =&\sum_{s=t}^TB_{ts}'\left(b_s - \sum_{r=1}^{t-1}B_{rs}L_r\gamma_r\right) = \sum_{s=t}^T\left(K_t^{-1}F_t\prod_{r=s+1}^tG_r\right)'\left[K_s^{-1}\left(y_s - F_t\prod_{r=1}^s(G_r)\gamma_0\right) - \sum_{r=1}^{s-1}K_r^{-1}F_r\prod_{i=r+1}^s(G_i)L_r\gamma_r\right].
\end{align*}
This allows us to write
\begin{align*}
  p(L_{1:T}|\gamma_{0:T},\cdots) &\propto \exp\left[-\frac{1}{2}\sum_{t=1}^T\left(\gamma_t'L_t'C_tL_t\gamma_t - 2c_t'L_t\gamma_t\right)\right].
\end{align*}
This looks like a normal kernel for $L_t\gamma_t$, but note that $c_t$ is a function of $L_{1:(t-1)}$. So the $L_t$'s are not conditionally independent, however it is immediately clear that 
\begin{align*}
  p(L_t|L_{1:(t-1)},\gamma_{0:T},\cdots) &\propto \exp\left[-\frac{1}{2}\left(\gamma_t'L_t'C_tL_t\gamma_t - 2c_t'L_t\gamma_t\right)\right]\\
  & \propto \exp\left[-\frac{1}{2}\left((L_t\gamma_t - C_t^{-1}c_t)'C_t(L_t\gamma_t - C_t^{-1}c_t) \right)\right]
\end{align*}
so that we can easily sample from $p(L_{1:T}|\gamma_{0:T},\cdots)$ as long as $p(L_t|L_{1:(t-1)},\gamma_{0:T},\cdots)$ is easy to sample from. While we have a normal kernel for $L_t\gamma_t$, it is not immediately clear whether $L_t$ will have a normal distribution. If we follow the approach of \cite{fruhwirth2008bayesian} and vectorize $L_t$ by stacking the nonzero elements of each column on top of each other, we will see that the nonzero elements of $L_t$ conditional on $L_{1:(t-1)},\gamma_{0:T},\cdots$ are normally distributed, again ignoring the prior. 
...
\end{align*}
In particular not that if $a$ is a column vector, $\vect(a)=a$ and $\vect(a')=\vect(a)'$. Suppose $p=q$ so that $A$ is square. Then there exists a unique $p(p+1)/2\times p^2$ matrix $S_p$ such that $\vech(A) = S_p\vect(A)$, called the elimination matrix. We list a few of the properties of $S_p$ here, but see \cite{magnus1980elimination} or \cite{magnus1988linear} for more details.
\begin{enumerate}
  ...
  \item If $A$ and $B$ are $p\times p$ matrices then $|S_p(A\otimes B)S_p'| = \prod_{i=1}^p|A^{(i)}||B_{(i)}|$ where $\otimes$ is the Kronecker product, i.e. $A\otimes B = (a_{ij}B)$, $A^{(i)}$ is the submatrix of $A$ obtained by deleting everything but the upper left $i\times i$ block, and $B_{(i)}$ is the submatrix of $B$ obtained by deleting everything but the lower right $(n-i+1)\times(n-i+1)$ block.
\end{enumerate}

Now we can apply this to our conditional distribution for $L_t|L_{1:(t-1)},\gamma_{0:T},\cdots $. Let $L_t^* = L_t - C_t^{-1}c_t$. Then
\begin{align*}
  p(L^*_t|L_{1:(t-1)},\gamma_{0:T},\cdots) & \propto \exp\left[-\frac{1}{2}\left((L^*_t\gamma_t)'C_t(L^*_t\gamma_t) \right)\right]\\
  &\propto \exp\left[-\frac{1}{2}\left(\vect(L^*_t\gamma_t)'C_t\vect(L^*_t\gamma_t) \right)\right]\\
  &\propto \exp\left[-\frac{1}{2}\left(\vect(L^*_t)'(\gamma_t' \otimes I_p)'C_t(\gamma_t' \otimes I_p)\vect(L^*_t) \right)\right]\\
  &\propto \exp\left[-\frac{1}{2}\left(\vect(L^*_t)'(\gamma_t\gamma_t' \otimes C_t)\vect(L^*_t) \right)\right]\\
  &\propto \exp\left[-\frac{1}{2}\left(\vech(L_t^*)'S_p(\gamma_t\gamma_t' \otimes C_t)S_p'\vech(L_t^* \right)\right].
\end{align*}
Here we use two properties of the Kronecker product $\otimes$. First for any matrices $A:p\times q$ and $B:q\times r$, $\vect(AB) = (B'\otimes I_p)\vect(A)$. Second, for any $p\times 1$ vector $\gamma$ and any $p\times p$ matrix $A$, $(\gamma'\otimes I_p)'A(\gamma' \otimes I_p) = \gamma\gamma'\otimes A$. Now converting back to $L_t$ we have
\begin{align*}
p(L_t|L_{1:(t-1)},\gamma_{0:T},\cdots) &\propto \exp\left[-\frac{1}{2}\left([\vech(L_t) - \vech(C_t^{-1}c_t)]'S_p(\gamma_t\gamma_t' \otimes C_t)S_p'[\vech(L_t)- \vech(C_t^{-1}c_t)] \right)\right].
\end{align*}
In other words
\begin{align*}
  \vech(L_t)|L_{1:(t-1)},\gamma_{0:T},\cdots \sim N\left(\vech(C_t^{-1}c_t), [S_p(\gamma_t\gamma_t'\otimes C_t)S_p']^{-1}\right).
\end{align*}
The fourth property of $S_p$ above ensures that this distribution is not singular.

So the conjugate prior on $L_t$ is a multivariate normal distribution. This seems a strange since we expect the diagonal elements of $L_t$ to be positive since they are standard deviations, but this is no problem as long as we view this prior as a clever trick for defining a prior on $W_t=L_t'L_t$ so that the sign doesn't matter. Strictly speaking here, we're subtly changed the definition of $L_t$ to the {\it signed} Cholesky decomposition of $W_t$, and thus subtly changed the defintion of the $\gamma_t$'s to take into account the signs of the diagonal elements of $L_t$. \cite{fruhwirth2008bayesian}, \cite{fruhwirth2011bayesian} and {\it CITE THE DYNAMIC PAPER SYLVIA IS WORKING ON WITH ANGELA} use this approach to choosing priors for the system (or hierarchical) variance when working with the scaled disturbances in dynamic and non-dynamic models, but only the first considers the covariance matrix case, rather than the scalar variance case, and that paper does not give the exlicit construction of the covariance matrix of $\vech(L_t)$ using the elimination matrix..
