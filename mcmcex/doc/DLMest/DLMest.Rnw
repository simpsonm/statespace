...
A well known method to estimate the DLM is via data augmentation (DA), as in \citet{fruhwirth1994data} and \citet{carter1994gibbs}. The basic idea is to implement a Gibbs sampler with two blocks. The generic DA algorithm with parameter $\phi$, augmented data $\theta$, and data $y$ obtains the $k+1$'st state of the Markov chain, $(\phi^{(k+1)},theta^{(k+1)})$, from the $k$'th state, $\phi^{(k)}$ as follows:
...
\begin{alg}GIS for DLMs.\label{alg:GIS}\\
  \begin{center}
    \begin{tabular}{lllll}
...
\begin{alg}Alternating for DLMs.\label{alg:ALT}\\
  \begin{center}
    \begin{tabular}{lllllll}
...
Similarly, from equation \eqref{dlmerrorjoint} we have the full condtional distribution of $W$ given $V$ and $\psi_{0:T}$:
\begin{align*}
