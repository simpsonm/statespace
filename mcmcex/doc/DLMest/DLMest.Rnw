...
\begin{alg}\label{pcisalg}
  \begin{align*}
    ...
    &[W|V^{(k+1)},\theta_{0:T},y_{1:T}] \to [\gamma_{0:T}|V^{(k+1)},W,\theta_{0:T},y_{1:T}] \to [W^{(k+1)}|V^{(k+1)},\gamma_{0:T},y_{1:T}]
    \end{align*}
\end{alg}
