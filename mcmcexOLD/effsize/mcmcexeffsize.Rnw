...
<<esplotT10, echo=FALSE, fig.cap=cap>>=
cap <- "Effective sample size in posterior sampler for a time series of length $T=10$ for $V$ and $W$ and for the state, scaled disturbance, and scaled error samplers. $X$ and $Y$ axes indicate the true values of $V$ and $W$ respectively for the simulated data. Note that the signal-to-noise ratio is constant moving up any diagonal. In the upper left the signal is high, in the lower right the noise is high. Also note that the actual sample size is $1500$."
...
<<esplotT100, echo=FALSE, fig.cap=cap>>=
cap <- "Effective sample size in posterior sampler for a time series of length $T=100$ for $V$ and $W$ and for the state, scaled disturbance, and scaled error samplers. $X$ and $Y$ axes indicate the true values of $V$ and $W$ respectively for the simulated data. Note that the signal-to-noise ratio is constant moving up any diagonal. In the upper left the signal is high, in the lower right the noise is high. Also note that the actual sample size is $1500$."
...
<<esplotT1000, echo=FALSE, fig.cap=cap>>=
cap <- "Effective sample size in posterior sampler for a time series of length $T=1000$ for $V$ and $W$ and for the state, scaled disturbance, and scaled error samplers. $X$ and $Y$ axes indicate the true values of $V$ and $W$ respectively for the simulated data. Note that the signal-to-noise ratio is constant moving up any diagonal. In the upper left the signal is high, in the lower right the noise is high. Also note that the actual sample size is $1500$."
...
<<maxcor, echo=FALSE>>=
mseff <- round(min(seff[[1]][[1]], seff[[1]][[2]], seff[[1]][[3]]),0)
mdeff <- round(min(deff[[1]][[1]], deff[[1]][[2]], deff[[1]][[3]]),0)
meeff <- round(min(eeff[[1]][[1]], eeff[[1]][[2]], eeff[[1]][[3]]),0)
...
We can do the same thing for some of the $\theta$'s. Figures
\ref{thetaplotT10}, \ref{thetaplotT100}, and \ref{thetaplotT1000}
contain plots the effective sample size for $\theta_0$ and $\theta_{T}$, for
...
usually large and doesn't seem to depend on the true parameter values,
...
models for the the state sampler is \Sexpr{mseff}. Likewise for the
scaled disturbance sampler the minimum effective sample size is
\Sexpr{mdeff} and the for scaled error sampler the minimum effective
sample size is \Sexpr{meeff}. These numbers are actually quite low. So
when looking at effective sample size, we see that for some $\theta$'s
some of the time, there can be some persistence in the chain, but it
isn't necessarily a large problem.


<<thetaplotT10, echo=FALSE, fig.cap=cap>>=
cap <- "Effective sample size in posterior sampler for a time series of length $T=10$ for $\\theta_0$ and $\\theta_T$ and for the state, scaled disturbance, and scaled error samplers. $X$ and $Y$ axes indicate the true values of $V$ and $W$ respectively for the simulated data. Note that the signal-to-noise ratio is constant moving up any diagonal. In the upper left the signal is high, in the lower right the noise is high. Also note that the actual sample size is $1500$."
...
<<thetaplotT100, echo=FALSE, fig.cap=cap>>=
cap <- "Effective sample size in posterior sampler for a time series of length $T=100$ for $\\theta_0$ and $\\theta_T$ and for the state, scaled disturbance, and scaled error samplers. $X$ and $Y$ axes indicate the true values of $V$ and $W$ respectively for the simulated data. Note that the signal-to-noise ratio is constant moving up any diagonal. In the upper left the signal is high, in the lower right the noise is high. Also note that the actual sample size is $1500$."
...
<<thetaplotT1000, echo=FALSE, fig.cap=cap>>=
cap <- "Effective sample size in posterior sampler for a time series of length $T=1000$ for $\\theta_0$ and $\\theta_T$ and for the state, scaled disturbance, and scaled error samplers. $X$ and $Y$ axes indicate the true values of $V$ and $W$ respectively for the simulated data. Note that the signal-to-noise ratio is constant moving up any diagonal. In the upper left the signal is high, in the lower right the noise is high. Also note that the actual sample size is $1500$."
...
Figures \ref{postplotT10}, \ref{postplotT100} and \ref{postplotT1000}
contain plots of a measure of the posterior correlations either $V$ or
$V$ and either the states ($\theta$'s), the scaled disturbances
($\gamma$'s), or  the scaled errors ($\psi$'s) for $T=10$, $100$, and
$1000$. In a given cell in one of plots the maximum in magnitude correlation
between either $V$ or $W$ and either the $\theta$'s, $\gamma$'s, or
$\psi$'s, depending on the table for the given true parameter values
and given length of the time series. In other words, for $T=10$ when
the true parameter values are $V=1$ and $W=1$ and for $T=10$, the largest
corrpelation between $V$ and any of the $\theta$'s is about $0.2$
while given the same parameter values, the maximum correlation between
$V$ and any of the $\psi$'s is about $-0.2$ or so. Using this rough
measure of dependence we see a few rough patterns begin to
emerge. When the signal-to-noise ratio is near 1, the
...
When looking at the $\gamma$'s, though, we see a
different picture. The correlation between $V$ and the $\gamma$'s is
the same as that of $V$ and the $\theta$'s: it's highest when the
signal-to-noise ratio is near 1, but only modestly high in absolute
terms. The correlation between $W$ and the $\gamma$'s is completely
different: it's highest when the signal-to-noise ratio is large, and
the correlation gets very close to 1.

The $\psi$'s have a similar pattern but reversed. The correlation
between $W$ and the $\psi$'s is highest when the signal-to-noise ratio
is near 1, but only modestly high. The correlation between $V$ and the
$\psi$'s is highest when the signal-to-noise ratio is large, though
this time the correlation doesn't get nearly as high.

There's one more pattern worth noting in these plots. As the sample
size increases, the posterior correlation uniformly increases (in
magnitude). We see the same pheomnenon in the correlation between $V$
and $V$ in Figure \ref{postplotVW}. The same basic general pattern is
seen here as well --- $cor(V,W)$ is highest when the signal-to-noise
ratio is near 1 and small otherwise. Note that $V$ and $W$ are
negatively correlated. This is precisely what we should expect from
the correlation between the two variance parameters given
the structure of the model. Such a sign pattern wasn't nearly as stark with
the correlation between either $V$ or $W$ and any of the states,
disturbances, or errors.

So in general, the correlation between the set of parameters is the
highest when the signal-to-noise ratio is near 1 and smaller
otherwise. The exception is if you transform the states into a scaled
version of themselves. Then the transformed version of the states
become correlated in the posterior with the thing you scaled with in
high signal-to-noise cases. In the case of the scaled disturbances, we
scaled by $\sqrt{W}$ so now $W$ and the scaled disturbances have a high
correlation in high signal-to-noise cases. In the case of the scaled
